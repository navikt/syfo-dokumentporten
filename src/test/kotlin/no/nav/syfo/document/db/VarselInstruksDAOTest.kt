            ...
            it("should insert and retrieve varsel instruks by document id") {
                runTest {
                    ...
                    val insertedVarselInstruks = testDb.connection.use { connection ->
                        val inserted = varselInstruksDAO.insert(connection, persistedDocument.id, expectedVarselInstruks)
                        connection.commit()
                        inserted
                    }
                    ...
                    retrievedVarselInstruks?.ressursId shouldBe expectedVarselInstruks.ressursId
                ...
                runTest {
                    ...
                    val exception = shouldThrow<SQLException> {
                        testDb.connection.use { connection ->
                            varselInstruksDAO.insert(connection, persistedDocument.id, varselInstruks())
                            varselInstruksDAO.insert(
                                connection,
                                persistedDocument.id,
                                varselInstruks(ressursId = "annen_ressurs"),
                            )
                        }
                    }
