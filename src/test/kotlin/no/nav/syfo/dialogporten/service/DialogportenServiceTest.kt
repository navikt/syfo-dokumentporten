...
class DialogportenServiceTest :
    DescribeSpec({
        ...
        describe("sendDocumentsToDialogporten") {
            ...
            context("when there is one document to send") {
                it("should send document to dialogporten with createDialog and update status to COMPLETED") {
                    ...
                    capturedDialog.party shouldBe
                        "urn:altinn:organization:identifier-no:${documentEntity.dialog.orgNumber}"
                ...
                it("should send document to dialogporten with addTransmission and update status to COMPLETED") {
                    ...
                    coEvery { documentDAO.getDocumentsByStatus(DocumentStatus.RECEIVED) } returns listOf(documentEntity)
                    coEvery { dialogportenClient.addTransmission(capture(transmissionSlot), any()) } returns
                        UUID.randomUUID()
                    coEvery { documentDAO.update(any()) } returns Unit
        ...
        describe("sendTransmissionOpenedActivities") {
            it("sends and marks an opened GUI transmission") {
                ...
                coEvery {
                    documentDAO.getDocumentsWithUnsentTransmissionOpenedActivities()
            ...
            it("does not send activities that are already handled or missing identifiers") {
                ...
                coEvery {
                    ...
                    sentDocument,
                    permanentlyFailedDocument,
                    documentWithoutDialogId,
                    documentWithoutTransmissionId,
                )
